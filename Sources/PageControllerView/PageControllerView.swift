...
import AppKit
/**
 * - Abstract: SwiftUI does not support tabview for macos. This class
 *             provides similar support. Primarly it solves swiping left and right
 *             for macOS onboarding
...
public struct PageControllerView: NSViewControllerRepresentable { // PageControllerRepresentable
    ...
    * - Description: The `dataSource` property holds an array of
    *                `NSPageController.ObjectIdentifier` which uniquely
    *                identifies each page in the `PageControllerView`. It is
    *                used by the page controller to manage the creation and
    *                arrangement of the page content.
    */
    ...
    * - Description: The `MakeView` typealias defines a closure type that takes
    *                an `NSPageController.ObjectIdentifier` and returns a SwiftUI
    *                view wrapped in `AnyView`. This closure is used to dynamically
    *                create the content for each page in the `PageControllerView`.
    */
   public typealias MakeView = (_ id: NSPageController.ObjectIdentifier) -> AnyView
   /**
    * Callback to make content for each page
    * - Description: here we make a callback, that takes index and returns
    *                View for the NSHostingController to use
    ...
    * - Description: Initializes a new PageControllerView with the specified
    *                data source, current page binding, effect view configuration,
    *                and a closure to create views for each page. This allows for
    *                the integration of a page controller within SwiftUI on macOS,
    *                providing functionality similar to a TabView with swipe
    *                navigation.
    * - Parameters:
    *   - dataSource: An array of identifiers for the pages. Each identifier
    *                 is used to create the corresponding page.
    *   - currentPage: A binding to a property that tracks the currently
    *                  selected page. The PageControllerView updates the
    *                  property as the user interacts with the interface.
    *   - makeView: A closure that takes an identifier and returns the view
    *               for the corresponding page. By default, it returns an
    *               empty view.
    */
